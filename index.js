...
var generateHTML = function (swaggerDoc, opts, options, customCss, customfavIcon, swaggerUrl, customSiteTitle, _htmlTplString, _jsTplString) {
  ...
  var htmlWithBaseURL = _htmlTplString.toString().replace(/{BASEURL}/g, baseURL);
  var htmlWithCustomCss = htmlWithBaseURL.toString().replace('<% customCss %>', customCss)
...
var setup = function (swaggerDoc, opts, options, customCss, customfavIcon, swaggerUrl, customSiteTitle) {
  ...
  return function (req, res) {
    if (req.swaggerDoc) {
      ...
      // res.send(reqHtml)
      res.writeHead(200, {
        'Content-Length': Buffer.byteLength(reqHtml),
...
//   var opts = options || {}
//   opts.index = false
//   return restify.static(swaggerUi.getAbsoluteFSPath(), opts)
// }
...
var swaggerAssetMiddleware = (options = {}) => {
  ...
  return (req, res, next) => {
    if(/(\/|index\.html)$/.test(req.path())) {
      return next()
    } else if(/(\/|swagger-ui-init\.js)$/.test(req.path())) {
      return next()
    } else {
